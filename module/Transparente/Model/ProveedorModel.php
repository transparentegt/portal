...
class ProveedorModel extends EntityRepository
{
    private function scrapPagedData()
    {
        $year     = date('Y');
        $postVars = [
            '_body:MasterGC$ContentBlockHolder$ScriptManager1' => 'MasterGC$ContentBlockHolder$UpdatePanel1|MasterGC$ContentBlockHolder$dgResultado$ctl54$ctl05',
            '__EVENTTARGET'                                    => 'MasterGC$ContentBlockHolder$dgResultado$ctl54$ctl05',
        ...
        for ($i = 1; $i <=2; $i++) {
            ...
            $html  = ScraperModel::getCachedUrl('http://guatecompras.gt/proveedores/consultaProveeAdjLst.aspx?lper='.$year, 'REST', $postVars);
